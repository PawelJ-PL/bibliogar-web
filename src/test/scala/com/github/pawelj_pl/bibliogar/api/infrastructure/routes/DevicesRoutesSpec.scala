...
class DevicesRoutesSpec extends WordSpec with Matchers with UserConstants with DeviceConstants {
  ...
  private def authToSession(session: UserSession): AuthInputs => TestEffect[Either[ErrorResponse, UserSession]] = (authInputs: AuthInputs) =>
  ...
  "Register device" should {
    ...
    "return data and logout cookie user" in {
      ...
      val request = Request[TestEffect](Method.POST, uri = ApiPrefix / "devices").withEntity(dto).withHeaders(Header("X-Api-Key", "something"))
      val (state, result) = routes.run(request).run(initialState).unsafeRunSync()
      result.status shouldBe Status.Ok
      result.cookies.find(_.name == "session") shouldBe Some(ResponseCookie(name = "session", content = "invalid", maxAge = Some(0)))
      result.as[DeviceRegistrationResp].runA(initialState).unsafeRunSync() shouldBe DeviceRegistrationResp(ExampleDevice.device_id, ExampleApiKey.apiKey)
  ...
  "Unregister device" should {
    ...
    "return 412" when {
      "API key not found in session" in {
        ...
        val result = routes(session).run(request).runA(initialState).unsafeRunSync()
        result.status shouldBe Status.PreconditionFailed
        result.as[ErrorResponse.PreconditionFailed].runA(initialState).unsafeRunSync() shouldBe ErrorResponse.PreconditionFailed("No API key found in session")
      }
      "API key is not device type" in {
        val session = ExampleUserSession.copy(apiKeyId = Some(ExampleApiKey.keyId))
        val initialState = TestState(devicesServiceState = DevicesServiceStub.DevicesServiceState(
          maybeError = Some(DeviceError.ApiKeyIsNotDeviceType(ExampleApiKey.keyId, KeyType.User))
        ))
        val request = Request[TestEffect](Method.DELETE, uri = ApiPrefix / "devices" / "this").withHeaders(Header("X-Api-Key", "something"))
        val result = routes(session).run(request).runA(initialState).unsafeRunSync()
        result.status shouldBe Status.PreconditionFailed
        result.as[ErrorResponse.PreconditionFailed].runA(initialState).unsafeRunSync() shouldBe ErrorResponse.PreconditionFailed("Not device API key")
      }
      "API key is not related to any device" in {
        ...
        val result = routes(session).run(request).runA(initialState).unsafeRunSync()
        result.status shouldBe Status.PreconditionFailed
        result.as[ErrorResponse.PreconditionFailed].runA(initialState).unsafeRunSync() shouldBe ErrorResponse.PreconditionFailed("API key is not assigned to any device")
      }
    }
  }
}
