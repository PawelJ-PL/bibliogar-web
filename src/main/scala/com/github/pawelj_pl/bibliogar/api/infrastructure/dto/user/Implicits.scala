...
trait PasswordImplicits {
  ...
  implicit val schemaForPassword: SchemaFor[Password] = SchemaFor(Schema.SString)
...
trait NickNameImplicits {
  ...
  implicit val schemaForNickname: SchemaFor[NickName] = SchemaFor(Schema.SString)
...
trait EmailImplicits {
  ...
  implicit val schemaForEmail: SchemaFor[Email] = SchemaFor(Schema.SString)
