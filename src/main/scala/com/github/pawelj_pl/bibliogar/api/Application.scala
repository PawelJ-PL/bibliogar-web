...
import com.github.pawelj_pl.bibliogar.api.infrastructure.messagebus.handlers.FcmSenderHandler
...
object Application extends IOApp {
  ...
  def serverResource: Resource[IO, Unit] =
    for {
      ...
      service <- Resource.liftF((scheduler concurrently server concurrently fcmHandler).compile.drain)
    } yield service
